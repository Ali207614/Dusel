...
const Order = () => {
  ...
  const getItems = (pagination) => {
    ...
    axios
      .get(
        url + `/api/items-return?offset=${get(pagination, 'page', 1)}&limit=${get(pagination, 'limit', limit)}&search=${get(pagination, 'value', '').toLowerCase()}&items=${state.map(item => `'${item.ItemCode}'`)}` + link,
      )
      .then(({ data }) => {
